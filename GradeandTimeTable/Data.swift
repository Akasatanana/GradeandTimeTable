    ...
    if let data = classdata.testlikeEvalItems{
        ...
        for item in evalitems{
            for ratio in item.gotRatios{
                gotRatio += ((Double(item.evalRatio) / Double(item.evalTime)) * (Double(ratio) / 100.0))
            }
        }
    }
    return gotRatio
...
func JudgeGradecolor (selectedclass: ClassData) -> Color {
    switch JudgeGrade(classdata: selectedclass){
    case "S":
        return Color.yellow
    case "A":
        return Color.red
...
func CalcRatio (item: AttendlikeEvalItem) -> Double{
    return ((item.evalRatio / Double(item.evalTime)) * Double(item.gotTime))
}

func CalcRatio (item: TestlikeEvalItem) -> Double{
    let mapped = item.gotRatios.map({(Double(item.evalRatio) / Double(item.evalTime)) * (Double($0) / 100.0)})
    return mapped.reduce(0.0){(res, ratio) -> Double in res + ratio}
}
